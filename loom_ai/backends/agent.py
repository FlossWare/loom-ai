...
class InMemoryAgentLoop:
    ...
    async def step(self, agent_id: str) -> AgentTurn:
        ...
        agent_ops = (
            list(self._operations.get(agent_id, []))
            + list(self._global_operations)
        )
        for op in agent_ops:
    ...
    async def pause(  # NOSONAR — async required by AgentLoop protocol
        self, agent_id: str,
    ) -> AgentCheckpoint:
        state = self._ensure_state(agent_id)
        state.status = "paused"
    ...
    async def resume(  # NOSONAR — async required by AgentLoop protocol
    ...
    async def cancel(  # NOSONAR — async required by AgentLoop protocol
        self, agent_id: str,
    ) -> bool:
    ...
    async def state(  # NOSONAR — async required by AgentLoop protocol
        self, agent_id: str,
    ) -> AgentState:
        return self._ensure_state(agent_id)

    async def register_operation(  # NOSONAR — async required by AgentLoop protocol
    ...
    async def list_operations(  # NOSONAR — async required by AgentLoop protocol
        self, agent_id: str,
    ) -> list[AgentOperation]:
