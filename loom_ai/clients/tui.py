...
async def _snapshot() -> dict[str, Any]:
    ...
    data: dict[str, Any] = {
        "url": getattr(client, "base_url", os.environ.get("LOOM_URL", "http://127.0.0.1:5000")),
...
def _text(win: curses.window, y: int, x: int, value: object, width: int, attr: int = 0) -> None:
    h, w = win.getmaxyx()
    if 0 <= y < h and x < w - 1:
        try:
...
def _header(win: curses.window, tabs: Any, theme_name: str, paused: bool) -> None:
    ...
    _text(win, 1, 2, "FlossWare Loom | Live Operator Control Panel", w - 4, curses.A_BOLD)
    _text(win, 2, 2, f"theme={theme_name}  refresh={'PAUSED' if paused else '2s'}", w - 4)
    if tabs:
        tabs.draw(win, 3, 2)
    else:
        _text(win, 3, 2, "1 Dashboard   2 Models   3 Health   4 Knowledge   5 Tools", w - 4)
    _text(win, 4, 0, "-" * min(w - 1, 78), w)


def _table(win: curses.window, headers: list[str], rows: list[tuple[object, ...]], y: int, height: int) -> None:
    if Table:
        Table(headers, rows).draw(win, y, 2, height)
        return
    _text(win, y, 2, "  ".join(headers), win.getmaxyx()[1] - 4, curses.A_BOLD | curses.A_UNDERLINE)
...
def _dashboard(win: curses.window, data: dict[str, Any]) -> None:
    ...
    rows = [
        ("Server", data.get("url", "")),
        ("Status", _status(data)),
        ("Ready", data.get("ready", {}).get("status", "unknown") if isinstance(data.get("ready"), dict) else data.get("ready")),
        ...
        ("Knowledge", _count(data.get("knowledge", {}), ("documents", "document_count", "total_documents", "count"))),
...
def _models(win: curses.window, data: dict[str, Any]) -> None:
    ...
    if not isinstance(models, list):
        ...
        return
    _text(win, 6, 2, f"MODELS ({len(models)})", w - 4, curses.A_BOLD | curses.A_UNDERLINE)
    for i, model in enumerate(models[:max(1, h - 10)]):
        _text(win, 8 + i, 2, f"{i + 1:>3}. {model}", w - 4)
    _text(win, h - 4, 2, "T changes theme. Model selector is available from the dropdown when the shared widgets package is installed.", w - 4)
...
def _knowledge(win: curses.window, data: dict[str, Any]) -> None:
    ...
    rows = list((str(k), str(v)[:100]) for k, v in value.items()) if isinstance(value, dict) else [("status", value)]
...
def _tools(win: curses.window, data: dict[str, Any]) -> None:
    ...
    _text(win, win.getmaxyx()[0] - 3, 2, f"Resources exposed: {len(data.get('resources', [])) if isinstance(data.get('resources'), list) else 'error'}", win.getmaxyx()[1] - 4)
...
def _run(stdscr: curses.window) -> None:
    ...
    tabs = Tabs(["Dashboard", "Models", "Health", "Knowledge", "Tools"]) if Tabs else None
    paused = False
    last_poll = 0.0
    ...
    while True:
        if not paused and time.monotonic() - last_poll >= 2:
            try:
                ...
                data = {"url": os.environ.get("LOOM_URL", ""), "health": {"error": str(exc)}, "at": time.strftime("%H:%M:%S")}
        ...
        if page == 0:
            ...
            _tools(stdscr, data)
        h, w = stdscr.getmaxyx()
        _text(stdscr, h - 2, 2, "1-5 tabs  ←/→ tabs  R refresh  P pause  T theme  Q quit", w - 4)
        _text(stdscr, h - 1, 2, f"Loom: {_status(data)}   poll={data.get('at', '--:--:--')}", w - 4, curses.A_BOLD)
        stdscr.refresh()
        key = stdscr.getch()
        ...
        if key in (ord("p"), ord("P")):
            ...
            selected = Dropdown([Option(x, x) for x in THEMES], THEMES.index(theme_name) if theme_name in THEMES else 0)
