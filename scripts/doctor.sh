...
if have podman; then rootless="$(podman info --format '{{.Host.Security.Rootless}}' 2>/dev/null || true)"; [[ "$rootless" == true ]] && ok Podman 'installed and rootless' || bad Podman 'installed but rootless mode is unavailable'; if podman run --rm quay.io/podman/hello >/dev/null 2>&1; then ok 'Podman smoke' 'container execution works'; else bad 'Podman smoke' 'container execution failed'; fi; else bad Podman 'not found'; fi
...
if [[ "$storage" == postgresql ]]; then if have pg_isready && pg_isready -h "$(value LOOM_PG_HOST)" -p "$(value LOOM_PG_PORT)" >/dev/null 2>&1; then ok PostgreSQL 'accepting connections'; else bad PostgreSQL 'configured but unavailable'; fi; else warn PostgreSQL "LOOM_STORAGE=$storage"; fi
provider="$(value LOOM_LLM_PROVIDER)"; base="$(value LOOM_LLM_BASE_URL)"; if [[ "$provider" == free || "$base" =~ ^https?://[^/]+ ]]; then ok 'LLM provider' 'configured'; else bad 'LLM provider' 'not configured'; fi
